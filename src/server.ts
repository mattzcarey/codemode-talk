...
export class CloudflareAPIAgent extends AIChatAgent<Env> {
  async onStart() {
...
callable()(
  CloudflareAPIAgent.prototype.connectServer,
...
export default {
  async fetch(request: Request, env: Env, _ctx: ExecutionContext) {
    ...
    // Execute user code via the codemode SDK in a sandboxed isolate (slide demo)
    if (url.pathname === "/api/execute" && request.method === "POST") {
      try {
        const { code } = (await request.json()) as { code: string };
        const executor = new DynamicWorkerExecutor({ loader: env.LOADER });
        const wrappedCode = `async () => { ${code} }`;
        const result = await executor.execute(wrappedCode, buildFns(tools));
